...
def get_q_multipliers_NN(t_reg, qnn_output, singleton_mask, qbin_ts_te=None):
    qnn = qnn_output + 0.0
    if qnn.ndim == 2:
        # set rate to NA before ts and after te
        for i in range(qnn.shape[0]):
...
def MCMC(all_arg):
    ...
    if it==0: # initialize chain
        ...
        if BDNNmodel in [2, 3]:
            ...
            q_multiA, denom_qA, norm_facA = get_q_multipliers_NN(cov_parA[5], qnn_output_unregA, singleton_mask, qbin_ts_te)
...
#                    trait_tbl_NN = add_diversity(trait_tbl_NN, ts, te, timesLA, time_vec, bdnn_rescale_div, n_taxa) # Why we miss the trait table?
            ...
            if BDNNmodel in [1, 3] and rr_bdnn <= 0.0:
                ...
                if num_processes == 0:
                    ...
                    t0 = time.time()
                    t1 = CustomThread(target=get_rate_BDNN_3D,
                                      args=(cov_par[3], trait_tbl_NN[0], cov_par[0], hidden_act_f, out_act_f))
                    t2 = CustomThread(target=get_rate_BDNN_3D,
                                      args=(cov_par[4], trait_tbl_NN[1], cov_par[1], hidden_act_f, out_act_f))
                    t1.start()
                    t2.start()
                    t1.join()
                    t2.join()
                    bdnn_lam_rates, denom_lam = t1._return
                    bdnn_mu_rates, denom_mu = t2._return
...
#                        print('n_S\n', n_S)
                    ...
                    if hasFoundPyRateC and model_cov==0 and use_ADE_model == 0 and not BDNNmodel in [1, 3]:
                        ...
                        if num_processes==0 or BDNNmodel:
